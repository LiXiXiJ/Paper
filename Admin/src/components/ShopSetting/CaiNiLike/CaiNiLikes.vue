    ...
    <div class="shoplists-list">
      <div class="shoplists-item" v-for="(item,i) in Search(keywords)" :key="item._id">
        ...
        <div class="item-title">
          ...
          <p class="title-p2">￥{{ item.like_price }}</p>
          <div class="item-button" v-show="flag">
            <input type="button" value="下架" @click="RemoveCaiNiLike(i)"></input>
          </div>
        </div>
      </div>
    </div>
...
<script>
    export default {
      ...
      created(){
        this.AdminGetCaiNiLikeShopList()
      },
      methods:{
        // 下架按钮
        changeFlag(){
        ...
        // 获取数据
        async AdminGetCaiNiLikeShopList(){
          const res = await this.$axios.get('/admingetcainilike',this.model);
          if (res.status === 200) {
            this.CaiNiLikeShopList = res.data
          }
        },
        // 查询商品
        Search(keywords){
          var newList = [];
          this.CaiNiLikeShopList.forEach(item => {
