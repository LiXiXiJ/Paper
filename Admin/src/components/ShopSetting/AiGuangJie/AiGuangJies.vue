      ...
      <div class="shoplists-item" v-for="(item,i) in Search(keywords)" :key="item._id">
        ...
        <div class="item-title">
          ...
          <div class="item-button" v-show="flag">
            <input type="button" value="下架" @click="RemoveAiGuangJie(i)"></input>
          </div>
        </div>
      </div>
    </div>
...
<script>
    export default {
      ...
      created(){
          this.AdminGetAiGuangJieShopList()
      },
      methods:{
        // 下架按钮
        changeFlag(){
        ...
        // 获取数据
        async AdminGetAiGuangJieShopList(){
          const res = await this.$axios.get('/admingetaiguangjie',this.model);
          if (res.status === 200) {
            this.AiGuangJieShopList = res.data
          }
        },
        // 搜索商品
        Search(keywords){
          var newList = [];
          this.AiGuangJieShopList.forEach(item => {
      ...
      //  下架
        RemoveAiGuangJie(i){
          this.AiGuangJieShopList.some((item,index) => {
            if (i === index) {
              this.AiGuangJieShopList.splice(i,1)
            }
          })
        }
      }
